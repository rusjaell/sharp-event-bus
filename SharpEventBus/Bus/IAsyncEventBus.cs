...
public interface IAsyncEventBus
{
    ...
    IAsyncEventSubscriber<TEvent> AddSubscriber<TEvent>(IAsyncEventSubscriber<TEvent> subscriber) where TEvent : class, IEvent;
    ...
    /// </summary>
    /// <typeparam name="TSubscriber">The subscriber type, implementing <see cref="IAsyncEventSubscriber{TEvent}"/>.</typeparam>
    ...
    TSubscriber RegisterSubscriber<TSubscriber, TEvent>(params object[] args)
        where TSubscriber : class, IAsyncEventSubscriber<TEvent>
